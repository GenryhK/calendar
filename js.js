...
document.body.appendChild(divMonth);}(enterYear,enterMonth));


let table = document.createElement(`table`);
for (t=0;t!=7;t++){let th = document.createElement(`th`);
...
if(firstNumber==0){for(let u=0;u!=colDayInMonth;u++){
	td[u].textContent=u+1} //пишем u +1 потому, что счет в массиве наичнается с нуля
}

else{
for(let u=1;u!=(colDayInMonth+firstNumber);u++){
	if(u>=firstNumber){td[u].textContent=(u+1)-firstNumber} //пишем u +1 потому, что счет в массиве наичнается с нуля
}}
...
function click(){let g = new Calendar (inputYear.value,inputMonth.value);
